...
const runGuida = function (config, args) {
    return new Promise((resolve) => {
        ...
        server.post("lockFile", async (request) => {
            await config.lockFile(request.body, request);
            request.respond(200);
        });

        server.post("unlockFile", async (request) => {
            await config.unlockFile(request.body, request);
            request.respond(200);
        });
...
module.exports = {
    ...
    findReferences: async (config, args) => {
        return await runGuida(config, { command: "find-references", ...args });
    },
    getHoverInformation: async (config, args) => {
        return await runGuida(config, { command: "get-hover-information", ...args });
    }
};