                            ...
                            case findLocalReferenceTarget requestLine requestChar values of
                                Just ( targetName, declarationRegion, body ) ->
                                    ...
                                    if List.isEmpty matches then
                                        Ok [ regionToLocation ( path, declarationRegion ) ]
                                        ...
                                        in
                                        Ok (argLoc ++ List.map (\region -> regionToLocation ( path, region )) matches)
                ...
                case Parse.fromByteString Target.GuidaTarget syntaxVersion Parse.Application content of
                    Ok (Src.Module _ _ _ _ imports values unions aliases _ _) ->
                        findSymbolAt root path requestLine requestChar imports values unions aliases
                            |> Task.map (Result.map regionToLocation)
                                                                ...
                                                                (\defContent ->
                                                                    ...
                                                                    case Parse.fromByteString Target.GuidaTarget defSyntaxVersion Parse.Application defContent of
                                                                        Ok (Src.Module _ _ _ defDocs _ defValues _ _ _ _) ->
                                                                            Ok (extractHoverFromDocs defRegion defDocs defValues)
...
findSymbolAtValue line char (A.At region (Src.Value _ _ srcArgs ( _, body ) maybeType)) =
    if regionContainsPosition region line char then
        let
            localVars : Dict String Name A.Region
            localVars =
...
findSymbolAtExpr line char localVars (A.At region expr_) =
    if regionContainsPosition region line char then
        case expr_ of
            ...
            Src.Let defs _ body ->
                let
                    updatedLocalVars : Dict String Name A.Region
                    updatedLocalVars =
                        List.foldl
                            (\( _, def ) acc ->
                                case A.toValue def of
                ...
                Nothing ->
                    ...
                    Outline.getAllModulePaths root
                        |> Task.andThen
                            (\allModulePaths ->
                                let
                                    allModuleHomes : Dict String String ( Pkg.Name, Utils.FilePath )
                                    allModuleHomes =
                                                                ...
                                                                Ok (Src.Module _ _ (A.At _ exports) _ _ _ targetUnions _ _ _) ->
                                                                    let
                                                                        ...
                                                                        findCtor maybeExposedList =
                                                                            List.stoppableFoldl
                                                                                (\(A.At _ (Src.Union ( _, A.At _ unionName ) _ ctors)) _ ->
                                                                                    case
                                                                                        List.stoppableFoldl
                                ...
                                let
                                    ...
                                    getProjectType =
                                        if pkgName == Pkg.dummyName then
                                            ...
                                            Task.succeed (Just (Parse.Package pkgName))
                                in
                                getProjectType
                                    |> Task.andThen
                                        (\maybeProjectType ->
