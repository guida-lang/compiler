...
lockFile path =
    T.IO (\_ s -> ( s, T.LockFile IO.pure path ))


unlockFile : T.FilePath -> IO ()
unlockFile path =
    T.IO (\_ s -> ( s, T.UnlockFile IO.pure path ))
...
dirWithCurrentDirectory dir action =
    dirGetCurrentDirectory
        |> IO.bind
            (\currentDir ->
                bracket_
                    (T.IO (\_ s -> ( s, T.DirWithCurrentDirectory IO.pure dir )))
                    (T.IO (\_ s -> ( s, T.DirWithCurrentDirectory IO.pure currentDir )))
                    action
            )
...
envLookupEnv name =
    T.IO (\_ s -> ( s, T.Pure (Dict.get name s.envVars) ))


envGetProgName : IO String
envGetProgName =
    T.IO (\_ s -> ( s, T.Pure s.progName ))


envGetArgs : IO (List String)
envGetArgs =
    T.IO (\_ s -> ( s, T.Pure s.args ))
...
forkIO ioArg =
    T.IO (\_ s -> ( s, T.ForkIO (\() -> IO.pure ThreadId) ioArg ))
                            ...
                            let
                                ( filteredSubscribers, readIndexes ) =
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                T.TakeMVarSubscriber_Stream_Maybe_DMsg takeIndex ->
                            ...
                            let
                                ( filteredSubscribers, readIndexes ) =
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                T.TakeMVarSubscriber_StreamResultBMsgBResultDocumentation takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                            ...
                            let
                                ( filteredSubscribers, readIndexes ) =
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                T.TakeMVarSubscriber_StreamResultBMsgBResultUnit takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                            ...
                            let
                                ( filteredSubscribers, readIndexes ) =
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                T.TakeMVarSubscriber_StreamResultBMsgBResultArtifacts takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                                    ...
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                ...
                                                T.TakeMVarSubscriber_BB_StatusDict takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                            ...
                            let
                                ( filteredSubscribers, readIndexes ) =
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                T.TakeMVarSubscriber_ListMVar takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                                    ...
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                ...
                                                T.TakeMVarSubscriber_BB_CachedInterface takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                                    ...
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                ...
                                                T.TakeMVarSubscriber_BED_StatusDict takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
                                    ...
                                    List.foldr
                                        (\subscriber ( filteredSubscribersAcc, readIndexesAcc ) ->
                                            case subscriber of
                                                ...
                                                T.TakeMVarSubscriber_Unit takeIndex ->
                                                    ( filteredSubscribersAcc, takeIndex :: readIndexesAcc )
...
builderHPutBuilder handle str =
    T.IO (\_ s -> ( s, T.HPutStr IO.pure handle str ))
...
binaryDecodeFileOrFail decoder filename =
    T.IO (\_ s -> ( s, T.BinaryDecodeFileOrFail IO.pure filename ))
        |> IO.fmap
...
binaryEncodeFile encoder path value =
    T.IO (\_ s -> ( s, T.Write IO.pure path (encoder value) ))
