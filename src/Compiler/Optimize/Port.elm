                    ...
                    toEncoder fieldType
                        |> Names.fmap
                            (\encoder ->
                                ...
                                in
                                Opt.Tuple (Opt.Str (Name.toElmString name)) value []
                            )
            in
            encode "object"
                |> Names.bind
                                                            ...
                                                            (let_ "a"
                                                                ...
                                                                (let_ "b"
                                                                    ...
                                                                    (List.foldr (\( i, index, _ ) -> letCs_ (JsName.fromIndex index) i)
                                                                        (Opt.Call list [ identity, Opt.List args ])
                                                                        indexedCs
                                                                    )
                                                                )
                                                            )
    ...
    Names.bind
        (\succeed ->
            let
                ...
                tuple : Opt.Expr
                tuple =
                    Opt.Tuple (toLocal 0) (toLocal 1) (List.indexedMap (\i _ -> toLocal (i + 2)) cs)
            in
            List.foldr (\( i, c ) -> Names.bind (indexAndThen i c))
                (indexAndThen (List.length cs + 1) lastElem (Opt.Call succeed [ tuple ]))
