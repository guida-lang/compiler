...
type alias ToError =
    T.CDN_Name -> T.CRA_Region -> T.CRA_Region -> T.CREC_Error


detect : ToError -> Tracker a -> R.RResult i w T.CREC_Error (Dict String T.CDN_Name a)
detect toError dict =
    Dict.foldl compare
        (\name values ->
            R.bind
...
detectHelp : ToError -> T.CDN_Name -> OneOrMore (Info a) -> R.RResult i w T.CREC_Error a
detectHelp toError name values =
    case values of
        OneOrMore.One { value } ->
            R.ok value
...
checkFields : List ( T.CRA_Located T.CDN_Name, a ) -> R.RResult i w T.CREC_Error (Dict String T.CDN_Name a)
checkFields fields =
    detect T.CREC_DuplicateField (List.foldr addField none fields)
...
checkFields_ : (T.CRA_Region -> a -> b) -> List ( T.CRA_Located T.CDN_Name, a ) -> R.RResult i w T.CREC_Error (Dict String T.CDN_Name b)
checkFields_ toValue fields =
    detect T.CREC_DuplicateField (List.foldr (addField_ toValue) none fields)
